...
namespace UncleCheese\MockDataObjects\Tasks;
...
use UncleCheese\MockDataObjects\Extensions\MockDataObject;
...
class MockDataBuilder
{
    ...
    protected function determineParentObj()
    {
        ...
        if (!$parentPage) {
            $parentPage = SiteTree::get()->filter(['Title' => trim($parent)])->first();
        }
        if (!$parentPage) {
