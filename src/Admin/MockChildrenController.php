...
namespace UncleCheese\MockDataObjects\Admin;
...
class MockChildrenController extends CMSMain
{
    ...
    public function MockChildrenForm()
    {
        ...
        $fields = new FieldList(
            ...
            new LiteralField('optionsheader',
                sprintf($numericLabelTmpl, 2, _t('MockData.CHOOSEOPTIONS', 'Choose options'))),
            new NumericField('Count', _t('MockData.HOWMANYPAGES', 'How many pages do you want to create?'), 10),
            new DropdownField('IncludeRelations', _t('MockData.RELATEDDATA', 'Related data'), [
                0 => _t('MockData.CREATEMOCKRELATED', 'Create mock data for relations'),
                1 => _t('MockData.NATIVEONLY', 'Only populate native fields')
            ]),
            new DropdownField('DownloadImages', _t('MockData.FILESANDIMAGES', 'Files and Images'), [
    ...
    public function doAddMockChildren($data, $form)
    {
        ...
        $this->response->addHeader(
            'X-Status',
            _t('MockData.CREATESUCCESS', 'Created {count} mock children under {title}',
                ['count' => $data['Count'], 'title' => $parentPage->Title])
        );
